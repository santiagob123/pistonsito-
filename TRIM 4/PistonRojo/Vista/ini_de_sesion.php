...
<script type="module">
    ...
    signInWithPopup(auth, googleProvider)
        .then(result => {
            ...
            if (user.email === "hm971535@gmail.com") {
                window.location.href = "../Vista/indexjv.php"; // Redirigir a la página de administrador
            } else {
                window.location.href = "indexUsu.php";  // Redirigir a la página de usuario normal
            }
        })
        .catch(error => {
            ...
            <form action="../Controlador/controladorUsuario.php" method="POST">
                ...
                <div class="form-grid">
                    ...
                    <div>
                        ...
                        <input type="text" id="numeroDocumento" name="numeroDocumento" placeholder="Número de documento" required pattern="\d{10}" minlength="6" maxlength="10">
                    </div>
                    <div>
                        <label for="userName">Nombre:</label>
                        <input type="text" id="userName" name="userName" placeholder="Nombre"   maxlength="20" required minlength="3" pattern="^[A-Za-záéíóúÁÉÍÓÚÑñ\s]+$">
                    </div>
                    <div>
                        <label for="userApellido">Apellido:</label>
                         <input type="text" id="userApellido" name="userApellido" placeholder="Apellido" maxlength="20"  required minlength="3" pattern="^[A-Za-záéíóúÁÉÍÓÚÑñ\s]+$">
                    </div>

                    <div>
                        <label for="userTelefono">Teléfono:</label>
                        <input type="tel" id="userTelefono" name="userTelefono" placeholder="Teléfono" maxlength="10"  required pattern="\d{10}" title="Debe ser un número de 10 dígitos">
                    </div>
                    <div>
                        <label for="userDireccion">Dirección:</label>
                        <input type="text" id="userDireccion" name="userDireccion" placeholder="Dirección" maxlength="15"  required minlength="5">
                    </div>
